...
export default function LogicTodo() {
  ...
  const handleErrorTodo = (input) => {
    ...
    const duplicated = todos.some(
      (todo) => todo.id !== editId && todo.title.toLowerCase() === input.toLowerCase()
    );
    if (duplicated) {
  ...
  const handleSaveEdit = () => {
    ...
    setTodos((prevTodos) =>
      prevTodos.map((todo) => (todo.id === editId ? { ...todo, title: trimmed } : todo))
    );
    setEditId(null);
    setEditTodo("");
    setTodoErr(null);
    ...
    setTodos((prevTodos) =>
      prevTodos.map((todo) => {
        if (todo.id === todoId) {
          return {
            ...todo,
            subtask: todo.subtask.map((sub) =>
              sub.id === subtaskId ? { ...sub, task: newSubtaskText } : sub
            ),
          };
        }
        return todo;
